...
//   const [imageAsset, setImageAssset] = useState(null);
...
//     setIsLoading(true);
//     const imageFile = e.target.files[0];
//     const storageReference = storageRef(storage, `Images/${Date.now()}-${imageFile.name}`);
//     const uploadTask = uploadBytesResumable(storageReference, imageFile);

//     uploadTask.on('state_changed', (snapshot) => {
//       const uploadProgress = (snapshot.bytesTransferred / snapshot.totalBytes) * 100;
//       // You can use `uploadProgress` if you want to show a progress bar or something
//     }, (error) => {
//       console.log(error);
//       setFields(true);
//       setMsg('Error while uploading : Try Again');
...
const CreateContainer = () => {
  ...
  const deleteImage = () => {
    setIsLoading(true);
    const deleteReference = storageRef(storage, imageAsset);
  ...
  const clearData = () => {
    ...
    setCalories("");
    setPrice("");
    setCategory("Select All Categories");
  };
